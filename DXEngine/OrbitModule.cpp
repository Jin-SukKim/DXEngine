...
namespace DE {
	...
	void OrbitModule::LoadFromJson(const json& data)
	{
		if (data.contains("center")) 
			m_center = JsonToVec3(data["center"]);
		if (data.contains("axis")) 
			m_axis = JsonToVec3(data["axis"]);
		if (data.contains("rotationRate")) 
			m_rotationRate = data["rotationRate"];
		if (data.contains("offset")) 
			m_initialOffset = data["offset"];
	...
	{
		if (m_hasRateCurve) 
			curves.emplace(ParticleCurveType::ORBIT_RATE, m_rateCurve);
	}
