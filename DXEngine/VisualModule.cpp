...
namespace DE {
	...
	{
		if (m_hasColorCurve) 
			curves.emplace(ParticleCurveType::COLOR, m_colorCurve);
		if (m_hasAlphaCurve) 
			curves.emplace(ParticleCurveType::ALPHA, m_alphaCurve);
		if (m_hasSizeCurve)  
			curves.emplace(ParticleCurveType::SIZE, m_sizeCurve);
	}
