...
namespace DE {
	...
	public:
		...
		// 배치 렌더링 지원
		const std::vector<std::unique_ptr<ParticleEmitter>>& GetMainEmitters() const { return m_emitters; }
		const std::vector<ParticleEmitter*>& GetActiveSubEmitters() const { return m_activeSubEmitters; }
		const auto GetMeshConstsAddress() const { return m_meshConsts.GetAddressOf(); }
	private:
		void Reset();
