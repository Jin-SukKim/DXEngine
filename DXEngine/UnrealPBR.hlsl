...
float3 GetNormal(PSInput input) {
    ...
    if (useNormalMap) {
        ...
        // Texture ��ǥ�迡 ���ǵ� Normal Vector�� World ��ǥ��� ��ȯ
        ...
        // Texture ��ǥ���� Normal Vector�� World ��ǥ��� ��ȯ��ķ� ��ȯ
...
float3 DiffuseIBL(float3 albedo, float3 normalWorld, float3 pixelToEye, float metallic) {
    ...
    // metallic�� 1.0�� ����������� kd ���� 0�� ������� metal�̸� diffuse�� �پ�鵵�� ����
...
float3 LightRadiance(Light light, float3 representativePoint, float3 posWorld, float3 normalWorld, uint shadowIdx) {
    ...
    float spotFactor = light.type & LIGHT_SPOT ?
                        ...
                        1.0; // Directional Light�̳� Point Light�� ���
    ...
    if (light.type & LIGHT_SHADOW) {
        ...
        // �������� �� ���� Texture ��ǥ ��� (Shadow Map�� Texture�̱� ������ ��ȯ)
...
float3 DirectLighting(Light light, float3 posWorld, float3 pixelToEye, float3 normalWorld, float3 albedo, float metallic, float roughness, uint shadowIdx) {
    ...
    // RepresentativePoint�� World ��ǥ�迡�� Shading�� Pixel ������ �������� ����ؼ� ã��
    float3 representativePoint = L + centerToRay * saturate(light.radius / length(centerToRay)); // Area Light�� ��ǥ�ϴ� ���� ����
    ...
    // ���� �ݼ��̸� kd = 0���� diffuse ���� Specular�� ���, ���� ��ݼ��̸� kd = 1�� diffuse�� ���
    float3 kd = lerp(float3(1, 1, 1) - F, float3(0, 0, 0), metallic);
      
    float3 diffuseBRDF = kd * albedo;
        
    // Sphere Normalization (Sphere Light) - Area Light�� Specular ������ �о����Ƿ� SphereNormalization���� ����
    float alpha = roughness * roughness;
    ...
    for (int i = 0; i < MAX_LIGHTS; ++i) {
        if (!(lights[i].type & LIGHT_OFF)) {
            ...
            if (abs(dot(float3(1, 1, 1), radiance)) > 1e-5) // radiance�� (0, 0, 0)�� ��� ������ ����
                ...
                // �׷��� Loop�� �ӽ÷� unroll�� ����� (���������� For loop�� Ǯ� 3�� �ݺ��ϴ� ������ �������)
